...
func main() {
	...
	if e = db.AutoMigrate(&model.User{}, &model.Mood{}, &model.MoodTagDef{}, &model.Assessment{}, &model.Journal{}, &model.UserAssessment{}); e != nil {
	...
	mtr := repository.NewMoodTagRepository(db)
	ar := repository.NewAssessmentRepository(db)
	jr := repository.NewJournalRepository(db)
	us := service.NewUserService(ur, logger)
	ms := service.NewMoodService(mr, mtr, logger)
	mts := service.NewMoodTagService(mtr, logger)
	...
	h := router.Handlers{User: handler.NewUserHandler(us, as, logger, cfg.JWTSecret, cfg.JWTIssuer), Mood: handler.NewMoodHandler(ms, logger), MoodTag: handler.NewMoodTagHandler(mts, logger), Assessment: handler.NewAssessmentHandler(as, logger), Journal: handler.NewJournalHandler(js, logger)}
